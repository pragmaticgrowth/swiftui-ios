...
`.animation(_)`, a missing/wrong `value:`, raw duration curves where spring presets fit, hand-rolled
loops that `PhaseAnimator`/`KeyframeAnimator` replace, an avoidable hand-spun SF Symbol where
`symbolEffect` fits, missing `contentTransition`, mis-wired `matchedGeometryEffect` hero transitions, and
motion that ignores Reduce Motion. Findings are written to disk in the toolkit's unified schema. This is
never a from-scratch animation generator.
...
- **Gesture-driven animation coupling** belongs to `audit-swiftui-touch-gestures`; **`TimelineView`-as-clock**
  drawing belongs to `audit-swiftui-drawing-canvas`. **UIKit/Core Animation** is out of scope entirely.
...
2. **Prefer the native primitive over the hand-rolled loop.** Spring presets over raw durations,
   `PhaseAnimator`/`KeyframeAnimator` over `Timer`/`repeatForever` chains (iOS 17), `symbolEffect` over a
   hand-spun SF Symbol (iOS 17), `contentTransition` on value changes (iOS 16). **All below or at the iOS-17
   project floor — these are *preference/native-idiom* judgments, not availability breaks** (see the
   inversion note below).
...
`id · tell · severity · fix · open reference`. Severities: **warning** (compiles but deprecated/non-native),
**advisory** (judgment / restraint). `flag` = show the ✅, dev applies. No defect here is a mechanical
single-answer rewrite, so **all are `fix_mode: flag-only`** (the animated value / curve / primitive is
always a judgment).

> **iOS-17-floor inversion (load-bearing).** On the sibling macOS toolkit these specialties carried
> **availability hard-fails** — the same spring/symbol/phase primitives sat *above* that platform's floor, so
> an ungated use was a *compile break* and the skill escalated to a gate finding. **On iOS the project floor
> is 17**, and **all of these ship at iOS 17 or earlier** (`symbolEffect`/`PhaseAnimator`/`KeyframeAnimator` = iOS 17;
> `contentTransition` = iOS 16; `matchedGeometryEffect` = iOS 14; spring presets reconcile to the iOS-17
> WWDC23 spring vocabulary). **So no gating finding fires here** — anim-03/04/05/06/07 collapse to pure
> *native-idiom* advisories (use the spring preset / use the primitive), never an availability escalation.
> The "gate it on the macOS arm" logic from the macOS skill is **deleted**, not ported.

| id | One-line tell | Sev | Fix | Reference |
|---|---|---|---|---|
| anim-01 | single-arg `.animation(x)` (deprecated implicit form) | warning | flag | `animation-currency-and-springs.md` |
...
| anim-04 | raw curve where a named spring preset `.bouncy`/`.smooth`/`.snappy` reads more native (no gate — all ship ≤ iOS 17) | advisory | flag | `animation-currency-and-springs.md` |
| anim-05 | `Timer`/`.repeatForever`/`repeatCount` loop where `PhaseAnimator`/`KeyframeAnimator` (iOS 17, ≤ floor) fits | advisory | flag | `phase-keyframe-and-symbol.md` |
| anim-06 | a hand-animated SF Symbol where `.symbolEffect` (iOS 17, ≤ floor) fits | advisory | flag | `phase-keyframe-and-symbol.md` |
| anim-07 | a value change (numeric `Text`, swapped symbol) animates without `.contentTransition` (iOS 16, ≤ floor) | advisory | flag | `phase-keyframe-and-symbol.md` |
...
**The spring-preset floor is UNVERIFIED-LOOKING but reconciled to the iOS-17 spring vocabulary** — the shipped
DocC renders `iOS 13.0` (a type-property-inheritance quirk where `.bouncy`/`.smooth`/`.snappy` inherit the
enclosing `Animation` floor); the WWDC23 "Animate with springs" provenance is the truth. Because the project
floor is **iOS 17**, the presets are unconditionally available either way — **the quirk never produces a
gating finding on iOS**; carry the WWDC23 provenance only to describe them honestly, never to gate.

## The real API, at a glance

**Real (modern, prefer):** `animation(_:value:)` (iOS 13+), `withAnimation(_:_:)`,
`matchedGeometryEffect(id:in:…)` (14+), `contentTransition(_:)` (16+), `PhaseAnimator` / `KeyframeAnimator`
/ `symbolEffect(_:options:value:)` (all **17+**), `Animation.bouncy`·`.smooth`·`.snappy` (WWDC23 spring
vocabulary, ≤ iOS-17 floor), `EnvironmentValues.accessibilityReduceMotion` (13+). **Deprecated:**
`animation(_:)` single-arg (the implicit form → `animation(_:value:)` / `withAnimation`).
...
1. **ORIENT.** `tree`/`find` the SwiftUI sources. Read the **deployment target**
   (`project.pbxproj` `IPHONEOS_DEPLOYMENT_TARGET`, or `Package.swift` `platforms:`). Record it. Note: with
   the toolkit's **iOS-17 floor** every animation primitive here is already available — anim-03…07 are
   *native-idiom* judgments, **not** availability checks. Only re-check the floor if the project's target is
   **below iOS 17** (then `contentTransition` 16, `symbolEffect`/`PhaseAnimator`/`KeyframeAnimator` 17 could
   genuinely need a gate; route a real gate finding to `audit-swiftui-availability-gating`).
...
3. **READ.** Open every located file **in full** — never pattern-match-and-patch blind. The `value:`
   binding, the `@Namespace` identity, and whether a `withAnimation` body mutates observed state are
   invisible to grep. Build a per-file inventory: each animated view + its trigger state + its motion role.
4. **DETECT.** Apply the index. Assign each candidate a **confidence**; report a finding **only at 100 %
   certainty** (a single-arg `.animation`, a hero transition with two different namespaces).
5. **VERIFY.** For anything ≤ ~70 % confidence run **both** sources. (a) **Practice** —
   `bash ${CLAUDE_PLUGIN_ROOT}/scripts/swiftui-ctx lookup <api> --platform ios --json` (e.g. `animation` →
   `consensus` **61 % `(_, value)` / 18 % `(_)`**, `symbolEffect` → `introduced_ios` 17,
   `matchedGeometryEffect` → 54 % `(id, in)`); for the deprecation rule also
   `swiftui-ctx deprecated <api> --json`. Read `consensus`, `deprecated`+`replacement`, `recommended`
   permalink, `introduced_ios`, `co_occurs_with`. (b) **Spec** — confirm via **Sosumi**
   (`curl -sSL https://sosumi.ai/<apple-path>`; never `WebFetch` `developer.apple.com`); protocol in
   `${CLAUDE_PLUGIN_ROOT}/references/_shared/sosumi-reference.md`. Cross-check `introduced_ios` against
   ...
   - **Deeper corpus evidence (the real Animation vocabulary).** Before flagging a raw curve where a
     preset fits (anim-03/04) run `bash ${CLAUDE_PLUGIN_ROOT}/scripts/swiftui-ctx lookup bouncy --platform ios --json`
     (+ `smooth`/`snappy`) for the real spring presets, durations, and overloads — e.g. `.bouncy` ships with
     the `(duration, extraBounce)` overload across the iOS corpus, so it is consensus, not exotic. For a
     custom transition (anim-08) run `bash ${CLAUDE_PLUGIN_ROOT}/scripts/swiftui-ctx recipe matchedGeometryEffect --json`
     for real conformers + permalinks (stable envelope, `next_actions` → `file <id>`); pair with
     `swiftui-ctx lookup animation --platform ios`.
...
7. **FIX.** This domain is **all `fix_mode: flag-only`** — leave each finding `open` with the ✅ in
   ...
   live swiftui-ctx **consensus shape `(_, value)` (61 %)**, grounded in real corpus code, not a placeholder:

   ```swift
   // ✅ Correct — swiftui-ctx `lookup animation --platform ios` consensus `(_, value)`
...
8. **DOUBLE-CHECK.** Re-confirm every citation still resolves and still says the floor you reported. If a
   recommended rewrite would introduce a new tell, loop that file back to DETECT.
...
| `spring-and-curves/` | a raw curve where a spring preset reads more native (anim-03, anim-04) |
| `phase-keyframe-symbol/` | a hand-rolled loop, an avoidable hand-spun symbol where `symbolEffect` fits, or a missing `contentTransition` (anim-05, anim-06, anim-07) |
...
| `references/phase-keyframe-and-symbol.md` | hand-rolled loops vs `PhaseAnimator`/`KeyframeAnimator`, `symbolEffect` use, missing `contentTransition` (anim-05/06/07) |
...
| Shared file | For |
|---|---|
| `${CLAUDE_PLUGIN_ROOT}/references/_shared/floors-master.md` | every floor/availability value (the reconciled truth; the spring-preset iOS-17 reconciliation) |
| `${CLAUDE_PLUGIN_ROOT}/references/_shared/hallucination-blacklist.md` | the canonical invented-name list |
| `${CLAUDE_PLUGIN_ROOT}/references/_shared/ios-gating.md` | the iOS-arm gating rule + wrong-arm failure (only if the project floor is < iOS 17) |
