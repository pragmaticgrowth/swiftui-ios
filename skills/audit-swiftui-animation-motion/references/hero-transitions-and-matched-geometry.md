...
`matchedGeometryEffect(id:in:)` (iOS 14.0+, below the iOS-17 floor — no gate) is the SwiftUI hero-transition
primitive: two views sharing an `id` in one `@Namespace` interpolate geometry when one replaces the other.
Two defects: a **mis-wired** match, and a **glass surface** that should morph with `glassEffectID` instead.
...
- **swiftui-ctx** (`swiftui-ctx lookup matchedGeometryEffect --platform ios --json`): `introduced_ios` **14**;
  consensus **54 % `(id, in)`** (44 % `(id, in, properties)`) — the bare id+namespace form is the canonical
  shape. FIX cites it + the `recommended` permalink (`swiftui-ctx file <id> --smart`).
...
- Practice: `swiftui-ctx lookup matchedGeometryEffect --platform ios --json` (consensus `(id, in)`,
  `co_occurs_with` including `animation`, `recommended` permalink).
