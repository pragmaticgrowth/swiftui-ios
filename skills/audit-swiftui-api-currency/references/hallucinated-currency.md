# Hallucinated APIs (iOS SwiftUI) — curr-13 / curr-14
...
## curr-13 · invented modifiers that do not exist on iOS
...
`.buttonStyle(.glass)` / `.buttonStyle(.glassProminent)` — all `iOS 26.0+`, gated with
`if #available(iOS 26.0, *)` below a 26 floor. For a non-glass invented helper (`.cardStyle()`) the
fix is the underlying real modifiers (`.clipShape`, `.background`, `.shadow`). `era: WWDC25/iOS-26`.
...
iPhone/iPad app uses the symbol → the hallucination finding is confirmed. A *real* symbol returns exit 0
with a `consensus` shape and a `recommended` permalink; that is the signal it is NOT hallucinated.

## curr-14 · real-but-platform-wrong: `.glassBackgroundEffect()`

```swift
content.glassBackgroundEffect()      // ❌ REAL symbol but visionOS-only — ABSENT on iOS
content.glassEffect()                // ✅ the iOS Liquid Glass call (iOS 26.0+)
```

`.glassBackgroundEffect()` exists, so `swiftui-ctx lookup` will *not* exit 3 — but its
`introduced_ios` / platform list is visionOS, and Sosumi shows no iOS availability arm. Flag it on
any iOS target. **Do NOT confuse with `glassEffect(_:in:)`, which IS `iOS 26.0+`** on iPhone/iPad —
never flag that as invented or platform-wrong. `era: WWDC25/iOS-26`.
...
- Apple — `glassEffect(_:in:)` / `GlassEffectContainer` (real Liquid Glass, `iOS 26.0+`): https://developer.apple.com/documentation/swiftui/view/glasseffect(_:in:) and https://developer.apple.com/documentation/swiftui/glasseffectcontainer (scraped 2026-06-06).
