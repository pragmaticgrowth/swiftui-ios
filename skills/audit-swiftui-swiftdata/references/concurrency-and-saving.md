...
**As of:** 2026-06-07 · iOS 17+ (`@ModelActor` is iOS 17.0+) · Xcode 26 SDK.
...
```swift
@ModelActor                                             // macro (iOS 17+): generates conformance to `protocol ModelActor`, giving this actor its own ModelContext
actor DataImporter {
...
> (**iOS 17.0+** per `floors-master.md`); the not-found is `low_corpus` (the iOS corpus
...
and changes made shortly before app backgrounding or termination are frequently **lost** — there are no
deinit/app-exit hooks. On iOS, a fast background → terminate cycle (e.g. the user force-quits) can drop
unsaved changes. Call `try modelContext.save()` explicitly after meaningful mutations (and on
`ScenePhase` `.background` transition), handling the thrown error.
...
// iOS: also save on ScenePhase .background, not only on a timer.
```
**Detection:** grep `sd-10` locates `modelContext` / `.mainContext` use; the agent READS to confirm a
mutation path with **no `try …save()` anywhere** (and no `ScenePhase.background` save). Severity
**advisory**, `fix_mode: flag-only`. The "auto-save drops a fast-background/terminate save" behavior is observed
practitioner experience, not a documented guarantee — carry it as `advisory` with `source: verify
against Xcode 26 SDK` unless Sosumi confirms a `save()` requirement for the background-transition path.
...
| https://developer.apple.com/documentation/swiftdata/modelactor | primary-doc | high | `@ModelActor` macro is iOS 17.0+; generates an actor owning its own `ModelContext`; `PersistentIdentifier` is the `Sendable` hand-off type. Confirmed 2026-06-07. |
