...
> The single highest API-hallucination domain in this skill. Liquid Glass shipped at **WWDC25 (June 9, 2025)** in **iOS 26** — so almost no pre-2025 training data holds the real API. AI fails two opposite ways: it **invents** a plausible name (`.glassBackground()`, `.liquidGlass()`, `LiquidGlassView`, `.material(.glass)`), or it knows the real `glassEffect()` but **breaks the design rules** (glass on content, glass-on-glass, no container, no availability gate).
>
> iOS-only. Every ✅ compiles against the iOS SDK and is gated on the **iOS** arm; macOS appears solely as ❌ contrast. Verified against the Apple docs snapshot of 2026-06-07 (iOS 26, Xcode 26, Swift 6.3.2).

**What it is.** Liquid Glass is a real-time light-lensing **material**, not a blur and not `.ultraThinMaterial` renamed. Content behind it stays visible (optically bent, not Gaussian-smeared), and the glass picks up tint from whatever sits beneath it. It is a **navigation-layer** material: it floats above content so content can shine through. Treat it as a tinted pane between two layers — you declare *placement, shape, tint*; the system owns lensing, motion highlights, and accessibility adaptation.

**Why AI gets this wrong.** (a) **Recency** — the names, the `Glass` struct, and the container model are all new in the iOS 26 SDK; a pre-mid-2025 corpus has zero exposure and confabulates in Apple's usual shape (`.somethingBackground()`, `.material(...)`). (b) **Word without discipline** — even models that ingested some 2025 content absorb the *word* "glass" but not the rules Apple attaches (navigation only, never glass-on-glass, group in a container because glass can't sample glass), producing real-name + wrong-placement that compiles but looks broken. (c) **No availability awareness** — every glass symbol is `iOS 26.0+`; with an iOS-17 deployment floor, shipping apps target iOS 17/18 *and* 26, so an ungated call is a build break.

---

## The real API surface

All of the following are confirmed `iOS 26.0+` from Apple primary docs (scraped 2026-06-06):
...
Glass.interactive(_:)                       // material reacts to a continuous touch
...
Verbatim signature (Apple): `nonisolated func glassEffect(_ glass: Glass = .regular, in shape: some Shape = DefaultGlassEffectShape()) -> some View`. The `Glass` presets are `.regular` (default), `.clear` (media-rich backgrounds only), and `.identity` (opts out of the glass material while keeping the modifier in the chain).

> **`Glass.interactive(_:)` — iOS 26.0+, touch-driven.** It is `iOS 26.0+` (confirmed in Apple's doc JSON `introducedAt`). On iOS the interactive variant tracks a **continuous touch** — the material scales/bounces/shimmers from the touch point as a finger presses and drags. Add it to glass controls the user directly manipulates (a floating action button, a draggable glass pill). (See mistake 8.)
...
❌ .glassBackgroundEffect()// EXISTS but is visionOS-ONLY — never on an iOS target
```

These are confident confabulations in the shape of older Apple APIs (`.background(.ultraThinMaterial)`). They fail to compile ("value of type 'some View' has no member 'glassBackground'"). The real modifier is **`glassEffect(_:in:)`**. `.glassBackgroundEffect()` is the trap that *does* exist — but only on visionOS — so it seeds the wrong guess for iOS code.
...
// ✅ CORRECT — the real modifier, gated on the iOS arm
...
`.glassBackgroundEffect()` exists but is **visionOS-only** — never use it on an iOS target.
...
Glass on a list row tells the visual system a row is structural chrome. It looks wrong and hurts legibility. Apple ships **zero** exceptions: in Mail/Photos/Music the toolbar and tab bar have glass; rows, message cells, and thumbnails never do.
...
### 5. No `#available` gating for iOS 17/18 users (the sharpest mistake)

```swift
// ❌ WRONG when the deployment target is below iOS 26 — won't compile
struct ToolbarView: View {
    var body: some View {
        HStack { /* … */ }.glassEffect()     // glassEffect is iOS 26.0+ only
    }
}
...
struct ToolbarView: View {
    var body: some View {
        if #available(iOS 26.0, *) {
            ...
            controls.background(.ultraThinMaterial)   // iOS 18 and earlier
        }
    }
}
...
Every Liquid Glass symbol is `iOS 26.0+`. With an iOS-17 deployment floor, the exact code that runs on an iOS-26 device fails to build for the iOS-17/18 target unless it is gated. Gate on the **iOS** arm — `#available(iOS 26.0, *)`, never `#available(macOS 26, *)` (a snippet copied from macOS docs never fires on iOS because the wildcard `*` already covers iOS, so the floor is never enforced). Use `@available(iOS 26.0, *)` to mark a whole type/function that only runs on 26+.
...
NavigationStack { List { /* … */ }.glassEffect() }                     // DON'T glass the nav bar/list
.toolbar { ToolbarItem { Button("Add", systemImage: "plus") {} } }
.toolbarBackground(.visible, for: .navigationBar)                      // also blocks glass

// ✅ CORRECT — rebuild on the iOS 26 SDK and standard chrome adopts glass automatically;
...
Recompiling against the iOS 26 SDK auto-adopts Liquid Glass for the **navigation bar, tab bar, toolbar, search field, sheets/popovers, and the iPad sidebar** — for free. Re-glassing them fights the system and risks glass-on-glass (mistake 3). Removing old overrides matters too: `.toolbarBackground(.visible)` and `.toolbarColorScheme(_:)` from the pre-26 era block glass rendering — delete them. (`.backgroundExtensionEffect()` is for extending *custom* hero/sidebar content under the floating glass — not for re-glassing standard chrome.)

### 8. Mixing `.regular` with `.clear`
...
// ✅ CORRECT — one variant per group; .interactive() IS available on iOS 26
@State private var hovered = false
Button("A") {}
    .glassEffect(.regular.interactive())                 // iOS 26: responds to the pointer — test the feel
    .onHover { hovered = $0 }                             // optional supplementary hover layer
    .scaleEffect(hovered ? 1.02 : 1.0)
    .animation(.easeInOut(duration: 0.2), value: hovered)
```

`.regular` and `.clear` have different characteristics and must never be mixed within a group; reserve `.clear` for media-rich, bold, bright content. `Glass.interactive(_:)` (`iOS 26.0+`) makes the material react to a **continuous touch** — add it to glass controls the user directly manipulates (a floating action button, a draggable pill). For morphing between sibling shapes, keep one variant, share a `@Namespace`, and use `glassEffectID(_:in:)` inside a `GlassEffectContainer` (all four required: same container, `@Namespace` + `glassEffectID`, `withAnimation`, and conditional render — not just `.opacity`/hidden).

---

## Toolbar & bar customization (iOS)

Standard chrome auto-glasses on an iOS 26 SDK rebuild; these modifiers tune it.

```swift
// EVERY ToolbarItem auto-glasses on iOS 26. To opt a group OUT of the shared
// glass platter (each item renders on its own / no shared background), use ToolbarItemGroup
...
.toolbar {
    ...
    .sharedBackgroundVisibility(.hidden)         // iOS 26.0+ ToolbarContent method (CONFIRMED)
}

// Separate toolbar items into distinct glass groups with ToolbarSpacer (iOS 26.0+):
.toolbar {
...
// Collapse the search field into the toolbar until invoked (iOS 26.0+):
.searchToolbarBehavior(.minimize)

// Soften the scroll-edge effect under the bar for a media-led screen:
.scrollEdgeEffectStyle(.soft, for: .top)         // iOS 26.0+ (CONFIRMED)
```

`.backgroundExtensionEffect()` extends custom hero/sidebar content under the floating glass chrome (it
mirrors the view into the safe-area edges) — use it on *your* content, not to re-glass system bars. Bar
corner radii and the safe-area inset under the glass bar are system-managed — don't override with custom
clipping or hard-coded safe-area numbers.

---

## Scroll-edge effects (the legibility-under-a-bar control)

`scrollEdgeEffectStyle(_:for:)` (iOS 26.0+) controls how a scrolling region meets the glass bar at its
edge. On iOS, `.automatic` resolves to **`.soft`** — content fades translucently under the bar glass.

```swift
// ✅ keep .soft (the iOS default) for media-/hero-led surfaces where the bar should feel
//    integrated with the scroll (media browsers, image-heavy layouts):
ScrollView { HeroContent() }
    .scrollEdgeEffectStyle(.soft, for: .top)
// ✅ switch to .hard for document-/list-oriented surfaces where a crisp toolbar boundary aids
//    readability (settings lists, text, tables); mix per edge:
List { /* … */ }
    .scrollEdgeEffectStyle(.hard, for: .top)
    .scrollEdgeEffectStyle(.soft, for: .bottom)
```

To toggle the effect on or off entirely rather than restyle it, use the sibling
`.scrollEdgeEffectHidden(_:for:)` (iOS 26.0+). The modifier `scrollEdgeEffectStyle(_:for:)` is confirmed
`iOS 26.0+`; the exact per-edge **default** (`.soft` on iOS) is observed behavior — confirm on a device
...
Two migration details that the auto-glass model drops:

- **`Tab(...)` + `@SceneStorage` for tab selection.** The new `Tab(...)` struct (iOS 18.0+) replaces the old `TabView`-style placement that the Liquid Glass tab look depends on. When you adopt it, tab-selection **state restoration is no longer automatic** — bind the selection to `@SceneStorage` so the active tab survives relaunch/window-restore.
- **Auto-removing the backward-compat `LabelStyle`.** A custom `LabelStyle` you add to keep toolbar items looking right on pre-Tahoe OSes is dead code once you raise the deployment target to iOS 26. Annotate it `@available(iOS, obsoleted: 26)` so the compiler **flags it for removal** the moment the floor moves to 26 — the same auto-removal idiom you use for any legacy pre-26 compatibility helper.
...
// ✅ CORRECT — backward-compat toolbar LabelStyle the compiler will force you to delete at the iOS 26 floor
@available(iOS, obsoleted: 26, message: "Drop once the deployment target is iOS 26 — glass styles items.")
...
The migration is **not backward-compatible**: apps targeting iOS 18 and earlier receive none of the glass behaviors and still need the conditional paths above.
...
- **Hallucinated names — hard fail:** `.glassBackground(` · `.liquidGlass(` · `LiquidGlassView` · `.material(.glass)` · `.background(.glass)`. None exist. `.glassBackgroundEffect(` on an iOS target → visionOS-only, flag it.
...
- **Double transparency:** `.glassEffect()` on a view that also has `.background(.ultraThinMaterial)` — stacking two transparency systems. Community-reported as crashing in early betas, but **unverified against the shipping iOS 26 SDK** — test before relying on it either way; regardless, use one system per view.

---

## Canonical pattern

Quote this verbatim. Custom floating glass controls on iOS 26, gated, container-grouped, one variant, supplementary hover feedback (add `.regular.interactive()` if you also want the material itself to react), single tinted primary:
...
- Apple — `glassEffect(_:in:)` (real modifier; `iOS 26.0+`; verbatim `nonisolated func glassEffect(_ glass: Glass = .regular, in shape: some Shape = DefaultGlassEffectShape()) -> some View`): https://developer.apple.com/documentation/swiftui/view/glasseffect(_:in:) (scraped 2026-06-06).
- Apple — `GlassEffectContainer` (`iOS 26.0+`; *"combines multiple Liquid Glass shapes into a single shape that can morph…"*): https://developer.apple.com/documentation/swiftui/glasseffectcontainer (scraped 2026-06-06).
- Apple — `GlassButtonStyle` + `.buttonStyle(.glass)` (`iOS 26.0+`): https://developer.apple.com/documentation/swiftui/glassbuttonstyle and https://developer.apple.com/documentation/swiftui/primitivebuttonstyle/glass (scraped 2026-06-06).
- Apple — `GlassProminentButtonStyle` + `.buttonStyle(.glassProminent)` (`iOS 26.0+`): https://developer.apple.com/documentation/swiftui/glassprominentbuttonstyle and https://developer.apple.com/documentation/swiftui/primitivebuttonstyle/glassprominent (scraped 2026-06-07).
- Apple — `Glass` and its members `Glass.identity`, `Glass.interactive(_:)`, `Glass.tint(_:)`, plus `glassEffectID(_:in:)`, `glassEffectUnion(id:namespace:)`, `glassEffectTransition(_:)`, `GlassEffectTransition.materialize`, `backgroundExtensionEffect()`, `scrollEdgeEffectStyle(_:for:)`, `scrollEdgeEffectHidden(_:for:)` (all `iOS 26.0+` per doc JSON `introducedAt`). `Glass.interactive(_:)` is confirmed `iOS 26.0+` and touch-driven on iOS. Roots: https://developer.apple.com/documentation/swiftui/glass , /glass/identity , /glass/interactive(_:) , /view/glasseffectid(_:in:) , /view/glasseffectunion(id:namespace:) , /view/glasseffecttransition(_:) , /swiftui/glasseffecttransition/materialize , /view/backgroundextensioneffect() , /view/scrolledgeeffectstyle(_:for:) , /view/scrolledgeeffecthidden(_:for:) (scraped 2026-06-07).
- Apple — toolbar visibility modifiers: `.sharedBackgroundVisibility(_:)` (a `ToolbarContent` method, `iOS 26.0+`, CONFIRMED) and `.toolbarBackgroundVisibility(_:for:)` (`iOS 18.0+`, CONFIRMED): https://developer.apple.com/documentation/swiftui/toolbarcontent/sharedbackgroundvisibility(_:) and https://developer.apple.com/documentation/swiftui/view/toolbarbackgroundvisibility(_:for:) (scraped 2026-06-07).
...
- Apple — `ToolbarSpacer` / `searchToolbarBehavior(_:)` (iOS 26.0+) for separating toolbar glass groups and collapsing the search field: https://developer.apple.com/documentation/swiftui/toolbarspacer and https://developer.apple.com/documentation/swiftui/view/searchtoolbarbehavior(_:) (scraped 2026-06-07).
- diskcleankit (dev.to) — *"Liquid Glass in Swift: official best practices for iOS 26"* (relays WWDC25 219/323 quotes: navigation-layer only, never glass-on-glass, group in `GlassEffectContainer`, never mix variants, tint primary only, free auto-adoption list): https://dev.to/diskcleankit/liquid-glass-in-swift-official-best-practices-for-ios-26-macos-tahoe-1coo (captured 2026-06-06).
...
**CONFIRMED `iOS 26.0+` (Apple doc JSON, scraped 2026-06-07):** `glassEffect(_:in:)`, `GlassEffectContainer`, `.buttonStyle(.glass)` / `.glassProminent`, `glassEffectID(_:in:)`, `glassEffectUnion(id:namespace:)`, `glassEffectTransition(_:)`, `GlassEffectTransition.materialize`, `Glass` / `.regular` / `.clear` / `.identity` / `.interactive(_:)` / `.tint(_:)`, `backgroundExtensionEffect()`, `scrollEdgeEffectStyle(_:for:)`, `scrollEdgeEffectHidden(_:for:)`, `.sharedBackgroundVisibility(_:)` (a `ToolbarContent` method). **CONFIRMED `iOS 18.0+`:** `.toolbarBackgroundVisibility(_:for:)`. `Glass.interactive(_:)` is `iOS 26.0+` and **touch-driven** on iOS. **NOT on iOS:** `.glassBackgroundEffect()` (visionOS-only). **UNVERIFIED — carry, do not assert; confirm on a device:** the per-edge scroll-edge *default* (`.soft` on iOS). **Community-reported, unverified against the shipping SDK:** the `.glassEffect()` + `.background(.ultraThinMaterial)` double-transparency crash. **Hallucinated — never emit:** `.glassBackground()`, `.liquidGlass()`, `LiquidGlassView`, `.material(.glass)`, `.background(.glass)`.
