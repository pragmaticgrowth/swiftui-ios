...
`--platform ios|macos|cross|any` (default ios) · `--offline` (catalog only, no live fetch) ·
`--catalog <dir>`.

## The 16 commands (full surface)
| Command | Use it to… | Key result fields / flags |
|---|---|---|
| `lookup <api>` | **START HERE** — the context pack for one API | `consensus:[{shape,pct}]` (% over ALL uses), `recommended:{id,repo,permalink,src,stars,author_authority,min_ios,score}`, `diverse[]`, `co_occurs_with[]`, `recipes[]`, `introduced_ios`, `deprecated`, `replacement?`, `doc`, `low_corpus`. A protocol/pattern (e.g. `NSViewRepresentable`) soft-redirects to its `recipe`. |
...
| `repo <owner/name>` | a corpus repo's production fingerprint | `{stars, platform, author_authority, min_ios_inferred, custom_components, counts:{dim→n}, deprecated_apis_used[]}` — a model of what a clean audit looks like. |
...
| `rankings [<dimension>]` | top corpus repos by a ranking | `by_total_unique_apis · by_modifier_breadth · by_custom_components · most_modern_stack` → `[{repo,stars,total_unique_apis,custom_components,min_ios}]`. |
...
- **VERIFY it exists / its floor** → `lookup <api>` (`introduced_ios`; an exit-3 = no real app uses it ⇒ corroborates a hallucination) + Sosumi `doc:`.
...
## Caveats
iOS-first corpus (`--platform macos` for macOS-only). `consensus` % is over all uses; `examples` is a curated
≤25/API sample (read the %, not the count). `low_corpus:true` (<10 repos) = thin evidence, lean on Sosumi.
The ranking is a heuristic; `recommended` ≈ "how a high-quality, currently-maintained iOS app writes it."
...
## Sources
- The bundled `swiftui-ctx` CLI + `catalog/` (319 iOS repos · SwiftSyntax · iOS 26 SDK) — `--help` / `stats`.
